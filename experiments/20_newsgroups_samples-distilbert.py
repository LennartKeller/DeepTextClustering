...
ex = Experiment('20newsgroups_samples-distilbert')
ex.observers.append(FileStorageObserver('../results/sacred_runs/20newsgroups_samples-distilbert/'))
...
def cfg():
    ...
    labels = ['rec.autos', 'sci.crypt', 'soc.religion.christian', 'rec.sport.baseball']
    result_dir = f"../results/20newsgroups_samples-distilbert/{strftime('%Y-%m-%d_%H:%M:%S', gmtime())}"
    early_stopping = True
...
def run(n_epochs,
    ...
    result_df.to_csv(os.path.join(result_dir, '20newsgroups_samples-distilbert.csv'), index=False)
