...
ex = Experiment('20newsgroups-strided-distilbert')
ex.observers.append(FileStorageObserver('../results/sacred_runs/20newsgroups-strided-distilbert/'))
...
def cfg():
    ...
    result_dir = f"../results/20newsgroups-strided-distilbert/{strftime('%Y-%m-%d_%H:%M:%S', gmtime())}"
    early_stopping = True
...
def run(n_epochs,
    ...
    for text, label in zip(train_texts, train_labels):
        spans = strided_spans(text, window=128, stride=96)
        span_labels = [label] * len(spans)
        strided_texts.extend(spans)
        strided_labels.extend(span_labels)
    train_texts = np.array(strided_texts)
    train_labels = np.array(strided_labels)
    ...
    for text, label in zip(val_texts, val_labels):
        ...
        strided_texts.extend(spans)
        strided_labels.extend(span_labels)
    val_texts = np.array(strided_texts)
    val_labels = np.array(strided_labels)
    ...
    result_df.to_csv(os.path.join(result_dir, '20_newsgroups-strided-distilbert.csv'), index=False)
